...
using namespace MAP_EASY_MERGE;
...
void SystemMap::Update(double dt) {
	...
	float longВistanceFromStar = 50000.f;
	...
	for (size_t index = 0; index < size; ++index) {
		...
		if ((body->_dataPtr->force.length() < minForce) && (star && (posStar - body->GetPos()).length() > longВistanceFromStar)) {
			delete body;
