...
using namespace MY_VEC_ARR;

SystemStackData SystemStackData::data;

Body::Body(const std::string& nameModel)
	: _model(Model::getByName(nameModel))
{}

Body::Body(const std::string& nameModel, const Math::Vector3& pos, const Math::Vector3& velocity, float mass, const std::string& name)
	: _mass(mass)
	, _velocity(velocity)
	, _model(Model::getByName(nameModel))
{
...
void SystemMap::Update(double dt) {
	size_t sizeData = SystemStackData::data.size;
	if (sizeData <= 1) {
		return;
	}
	
	Body::Data* datas = SystemStackData::data.bodies;
	...
	auto getForce = [&](size_t statIndex, size_t endIndex) {
		for (size_t index = statIndex; index <= endIndex; ++index) {
			...
			for (size_t otherIndex = 0; otherIndex < sizeData; ++otherIndex) {
				...
				float dist = Math::length(gravityVec);
	...
	for (Body* body : _bodies) {
		...
		Math::Vector3 acceleration = body->_dataPtr->force / body->_mass;
		Math::Vector3 newVelocity = acceleration * static_cast<float>(dt);
...
void SystemMap::Save() {
	...
	for (Body* body : Objects()) {
		...
		Math::Vector3 velocity = body->_velocity;
...
void SystemMap::DataAssociation() {
	...
	Body::Data* bodies = SystemStackData::data.bodies;
	...
	SystemStackData::data.size = size;
}

Math::Vector3 SystemMap::CenterMass() {
	float sumMass = 0;
