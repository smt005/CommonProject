...
public:
	struct Data {
		...
		Data() = default;
		Data(const float _mass, Math::Vector3&& _pos)
			: mass(_mass)
			, pos(_pos)
			, force(0, 0, 0)
		{}
	...
	Body(const std::string& nameModel, const Math::Vector3& pos, const Math::Vector3& velocity, float mass, const std::string& name);
	~Body();
